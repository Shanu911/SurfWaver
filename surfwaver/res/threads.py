...
class DispersionThread(QtCore.QThread):
    ...
    sindx = QtCore.pyqtSignal(list)
    endprocss =  QtCore.pyqtSignal()
    def __init__(self, dispdata, gatherDict, traceDict, filterWidth, sint,parent=None):
        super().__init__(parent)
    ...
    def run(self):
        ...
        if self.checkdataoOk():
            ...
            mintrace = self.dispdata["mintrace"]
            settings = swprocess.Masw.create_settings_dict(workflow="time-domain",
                    ...
                    transform=self.dispdata["transform"], fmin=self.dispdata["fmin"], fmax=self.dispdata["fmax"], pad=True, df=0.5,
            ...
            for i in range(len(subps)):
                if len(self.gatherDict[subps[i]]) > (mintrace-1):
                    wavefieldtransforms.append(modMasw.run(cmpcc_strm=self.traceDict[i], 
                                sloc=subps[i], rloc= self.gatherDict[subps[i]], dt=self.dt, settings=settings))
    ...
    #       #   # Dispersion curve figure
                    ...
                    wavefieldtransform.plot(fig=fig4, ax=ax4, normalization=wavefield_normalization, 
                            nearfield=nearfield, fname=fnames[1], filterwidth=self.filterwidth)
                    xlim = ax4.get_xlim()
                    ylim = ax4.get_ylim()
                    ...
                    peak = swprocess.peaks.Peaks(wavefieldtransform.frequencies,
                                     wavefieldtransform.find_peak_power(by="frequency-maximum",width=self.filterwidth),
...
class fftImgThread(QtCore.QThread):
    ...
    def run(self):
        ...
        for i, src in enumerate(sources):
            ...
            ax.plot(freqx, 20*np.log10(ffttraceDataMeg0/ max(ffttraceDataMeg0)), "k", label=f'Average')
            ax.grid(linestyle="--", linewidth="0.3", color="g")
            ...
            fname = self.flder+labels[2]+str(src)+".jpg"
            fontsz=8
            if len(rcvrs) > 18: fontsz=6
            pos = ax.get_position()
            ax.set_position([pos.x0, pos.y0, pos.width * 1, pos.height])
            ax.legend(loc='center right', bbox_to_anchor=(1.23, 0.5), prop={'size': fontsz})
            fig.tight_layout()
            fig.savefig(fname)
