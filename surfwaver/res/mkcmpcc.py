        ...
        for file in self.files:
            if os.path.getsize(file)<5000000:  #only active file
                ...
                if segyfile:
                    ...
                    recvr0_loc = so_loc[-1] - float(f[0].stats.segy.trace_header.group_coordinate_x)
                    ...
                    recvr0_loc = so_loc[-1] - float(f[0].stats.seg2["RECEIVER_LOCATION"])
                ...
                for s in range(1,n_tr):
                    ...
                    while (b+s)<=n_tr-1:
                        if sorceAtLeft:
                            ...
                            xcorr = np.correlate(f[b].data, f[b+s].data, mode='full')
